...
class UserPostListView(ListView):
    ...
    def get_queryset(self):
        ...
        )  # kwargs rep query parameters
        return Post.objects.filter(author=user).order_by(
            "-date_posted"
        )  # the overrode ordering
...
class PostUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        ...
        )  # checks that posts authors can update posts. raises a 403 forbidden for not authors


class PostDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    model = Post
    success_url = (
        "/"  # this redirects back to homepage when a post is succesfully deleted
    )
...
def post_share(request, post_id):
    ...
    if request.method == "POST":
        ...
        form = EmailPostForm()
    return render(
        request, "blog_app/post_share.html", {"post": post, "form": form, "sent": sent}
    )
